...
disruption_days = np.random.randint(5, 15, size=num_products)
disruption_start_days = np.random.randint(1, 15, size=num_products)
disruption_effect = np.random.randint(1, 3, size=num_products)
...
data = {
    ...
    'Disruption_Start_Day': np.repeat(disruption_start_days, days),
    'Disruption_Duration': np.repeat(disruption_days, days),
    'Disruption_Effect': np.repeat(disruption_effect, days)
}
df_inventory = pd.DataFrame(data)

# Adjust inventory levels for disruptions
df_inventory['Disruption'] = ((df_inventory['Day'] >= df_inventory['Disruption_Start_Day']) &
                              (df_inventory['Day'] < (df_inventory['Disruption_Start_Day'] + df_inventory['Disruption_Duration'])))
df_inventory['Adjusted_Lead_Time'] = np.where(df_inventory['Disruption'], 
                                              df_inventory['Lead_Time_Days'] + df_inventory['Disruption_Effect'], 
...
# Save to CSV
df_inventory.to_csv('synthetic_inventory_with_disruptions.csv', index=False)
