...
safety_stock = np.random.randint(10, 30, size=num_products)
...
data = {
    ...
    'Safety_Stock': np.repeat(safety_stock, days),
...
df_inventory['Reorder_Needed'] = df_inventory['Ending_Inventory'] < (df_inventory['Reorder_Point'] + df_inventory['Safety_Stock'])
df_inventory['Reorder_Quantity'] = np.where(df_inventory['Reorder_Needed'], df_inventory['Reorder_Quantity'], 0)

# Save to CSV
df_inventory.to_csv('synthetic_inventory_with_safety_stock.csv', index=False)
